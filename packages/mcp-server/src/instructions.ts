...
async function fetchLatestInstructions(stainlessApiKey: string | undefined): Promise<string> {
  ...
  const response = await fetch(
    readEnv('CODE_MODE_INSTRUCTIONS_URL') ?? 'https://api.stainless.com/api/ai/instructions/modern-relay',
    {
      method: 'GET',
  ...
  if (!response.ok) {
    ...
    instructions = `
      This is the modern-relay MCP server. You will use Code Mode to help the user perform
