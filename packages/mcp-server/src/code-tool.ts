...
import { ClientOptions } from 'modern-relay';

const prompt = `Runs JavaScript code to interact with the Modern Relay API.
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    headers: {
      ...
      'x-stainless-mcp-client-envs': JSON.stringify({
        MODERN_RELAY_API_KEY: readEnv('MODERN_RELAY_API_KEY') ?? client.apiKey ?? undefined,
        MODERN_RELAY_BASE_URL: readEnv('MODERN_RELAY_BASE_URL') ?? client.baseURL ?? undefined,
      }),
    },
    body: JSON.stringify({
      project_name: 'modern-relay',
      code,
      intent,
      client_opts: {},
